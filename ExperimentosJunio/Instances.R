...
for (iteracion in numMaquina){
   print(iteracion)
   for (iter in numIns){
      print(iter)
      
      BatchTime <- read.table(paste("BatchTime_50_50_10_",iteracion,"_",iter,".txt", sep = ""))
      PPMMA <- read.table(paste("InfPPMM_50_50_10_",iteracion,"_",iter,".txt", sep = ""))
      ordenMoldes <- read.table(paste("ordenMoldes_50_50_10_",iteracion,"_",iter,".txt", sep = ""))
      productos <- read.table(paste("Piezas_50_50_10_",iteracion,"_",iter,".txt", sep = ""))
      demanda <- read.table(paste("Demanda_50_50_10_",iteracion,"_",iter,".txt", sep = ""))
      importancia <- read.table(paste("Importancia_50_50_10_",iteracion,"_",iter,".txt", sep = ""))
      ...
      periodos <- 288 #periodos (5 min) para las 24 horas del d?a 
      periodoTiempo <- periodos*productos[5,1]
      time <- 300 #un minuto tiene 60 segundos (por 5 min)
        ...
        for (m in 1:productos[5,1]){
          ...
          if (length(aux) > 0){
            ...
            while (j <= periodos){
              ...
              if (totalPiezas1[b,3] == trabajo[i,j+periodos*(m-1)] && trabajo[i,j+periodos*(m-1)] != -1){
      ...
      print(paste("TotalPiezas",length(totalPiezas[,1])))
      print(paste("TotalPiezasSobrantes",length(totalPiezas1[,1])))
      ...
      write.table(Instancia,file= paste("Instancia_Prueba_50_50_10_",iteracion,"_",iter,".txt",sep = ""), sep=" ", col.names=FALSE, row.names=FALSE,quote=FALSE)
